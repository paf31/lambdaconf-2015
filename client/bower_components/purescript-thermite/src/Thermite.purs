...
render :: forall props eff. ComponentClass props (dom :: DOM | eff) -> props -> Eff (dom :: DOM | eff) Unit
render cc props = do
  body <- documentBody
  renderTo body cc props

-- | Render a component class to the specified node.
renderTo :: forall props eff. Node -> ComponentClass props (dom :: DOM | eff) -> props -> Eff (dom :: DOM | eff) Unit
